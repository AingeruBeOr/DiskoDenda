...
public class DB {
  ...
  //************************************************DELETE************************************************************
    ...
    private void lekuakGiratikKendu() {
    	...
    	do {
    		try {
    			System.out.println("Sartu taldearen kodea:");
    			taldeKode = Integer.parseInt(br.readLine());
	            PreparedStatement ps = konexioa.prepareStatement(
	            		"SELECT Talde.izena, LEKUAN_JO.hasData, LEKUAN_JO.herrialdea, LEKUAN_JO.Hiria, LEKUAN_JO.Izena " + 
	            		"FROM  TALDE, LEKUAN_JO " + 
	            		"WHERE TALDE.kodea= ? and TALDE.kodea = LEKUAN_JO.taldek "
	            );
	            ps.setInt(1,taldeKode);
	            ResultSet rs = ps.executeQuery();
	            	...
	            	do {
	            		try {
	            			do {
				            	...
				            	PreparedStatement ps2 = konexioa.prepareStatement(
				            			"DELETE FROM Lekuan_Jo " +
				            			"WHERE Hiria = ? AND  Herrialdea = ? AND HasData = ? AND TaldeK = ? AND Izena=?"
				            	);
				            	ps2.setString(1,hiriIzen);
				            	ps2.setString(2,hiriHerrialde);
				            	ps2.setString(3, hasData);
				            	ps2.setInt(4, taldeKode);
				            	ps2.setString(5,lekua);
				            	ps2.executeUpdate();
  ...
  //************************************************SELECT************************************************************
    ...
    private void taldearenGirakErakutsi(){
    	...
    	do {
			try {
	            ...
	            PreparedStatement ps2 = konexioa.prepareStatement("SELECT * FROM TALDE WHERE IZENA LIKE ?");
	            ps2.setString(1,taldeIzen+"%");
	            ResultSet rs = ps2.executeQuery();
	            if(rs.next()) {
	            	ondo=true;
	            	saiakera=3;
	            }else System.out.println("Ez da existitzen taldea. Bilatu beste bat");
			}catch(Exception e) {
    			salbuespenaTratatu(e);
    		}
    		saiakera++;
	    ...
	    if(ondo) {
	    	...
	    	do{
		    	try{
		            ...
		            PreparedStatement ps = konexioa.prepareStatement(
		                    "SELECT  DISKO.KODEA, TALDE.IZENA, DISKO.Izena, ABESTIA.* " +
		            ...
		            ResultSet rs = ps.executeQuery();
		            while(rs.next()){
		                System.out.println(rs.getString(2) + "\t" +rs.getString(3) + "\t" + rs.getInt(5) + " " + rs.getString(4));
		                ondo=false;
		            }
    ...
    private void diskoenPrezioakErakutsi() {
    	...
    	do {
    		try {
    	        ...
    	        PreparedStatement ps = konexioa.prepareStatement(
    	        		"SELECT TALDE.IZENA, DISKO.izena, DISKO.prezioa " + 
    	        		"FROM TALDE, DISKO " + 
	            			...
	            			System.out.println("Aukeratu horietako bat: ");
	    	 	            ...
	    	 	            if(rs2.next()) {
	    	 	            	ps2 = konexioa.prepareStatement(
	    	   		                    "SELECT TALDE.IZENA, SUM(LEKUAN_JO.SaldutakoSarrerak * LEKUAN_JO.Prezioa) " +
	    	   		                    "FROM TALDE, LEKUAN_JO " +
	    	   		                    "WHERE TALDE.izena like ? and " +
	    	    	 	            ...
	    	    	 	            if(rs2.next()) {
	    	    	 	            	System.out.println(rs2.getFloat(1)+"\t"+rs2.getFloat(2)+"€");
	    	    	 	            	saiakera=3;
	    	    	 	            }
	    	 	            }else System.out.println("Aukeratu aukera egokia emandako daten artean.");
	        			} catch (Exception e) {
	        				salbuespenaTratatu(e);
	        			}
	            		saiakera++;
    	...
    	do {
    		try {
	            ...
	            if(rs.next()) {
	    			...
	    			saiakera=3;
	            }else {System.out.println("Ez da existitzen diskorik izen horrekin");}
    		}catch(Exception e) {
    			salbuespenaTratatu(e);
    		}
    		saiakera++;
    ...
    /*int saiakera=0;
	    do{
	    	try{
	            ...
	            PreparedStatement ps = konexioa.prepareStatement(
	                    "SELECT  TALDE.IZENA, LEKUAN_JO.Izena, LEKUAN_JO.HIRIA, LEKUAN_JO.HERRIALDEA, LEKUAN_JO.Prezioa " +
	                    "FROM  TALDE,  LEKUAN_JO " +
    ...
    private void giraPrezioekinErakutsi() {	
    	boolean daude=false;
    	int saiakera=0;
    	String taldeIzen="";
    	do {
			try {
	            ...
	            taldeIzen = br.readLine();
	            PreparedStatement ps = konexioa.prepareStatement(
	            		"SELECT distinct Talde.izena, LEKUAN_JO.hasData " + 
	            		"FROM  TALDE, LEKUAN_JO " + 
	            		"WHERE TALDE.izena like ? and TALDE.kodea = LEKUAN_JO.taldek "
	            );
	            ps.setString(1,taldeIzen+"%");
	            ResultSet rs = ps.executeQuery();
	            System.out.println("Hauek dira taldeak egiten dituen giren hasiera data:");
	            ...
	            if(daude) {
	            	...
	            	do {
	            		try {
	            			...
	            			}else System.out.println("Aukeren artean dagoen disko bat aukeratu.");
	        			} catch (Exception e) {
	        				salbuespenaTratatu(e);
	        			}
	            		saiakera++;
	        		} while (saiakera < 3);
	            }else System.out.println("Ez da talde hori existitzen, beste bat bilatu.");
    		}catch(Exception e) {
    			salbuespenaTratatu(e);
    		}
    		saiakera++;
	            ...
	            if(daude) {
	            	do {
	            		try {
	            			...
	            			}else System.out.println("Sartutako gira ez dago emandako aukeren artean, saiatu berriz ere.");
	        			} catch (Exception e) {
	        				salbuespenaTratatu(e);
	        			}
	            		saiakera++;
